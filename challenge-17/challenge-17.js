...
*/
  ...
  console.log(
    text.replace(/Manuel Marques de Sousa/g, "Fernando Renan Cardoso da Silva")
  );
...
*/
  ...
  console.log(
    text.replace(/O Centauro de Luvas/g, function (phrase) {
      ...
      //console.log(  e.replace(ct, ct.toUpperCase()))
    })
  );
...
*/
  ...
  function getMonthNumber(monthName) {
    ...
    for (let index = 0; index < month.length; index++) {
      ...
      let monthConv = index + 1;

      if (monthName === element) {
        let prefix = "0";
        if (monthConv < 10) {
          monthNumber = prefix.concat(monthConv);
        } else {
          monthNumber = monthConv;
        }
      }
    }
    return monthNumber
    ...
    // console.log(
    //   `O mês de ${monthName} é representado pelo número ${monthNumber}`
    // );
  }
